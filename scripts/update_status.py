...
PAGE_SIZE = 100
ACCESSION_RE = re.compile(r"^(PXD|PRD|MSV|RPXD)\d+$")
...
def blocked_ids() -> set[str]:
    ...
    if summary.exists():
        ids |= set(re.findall(r"\bPXD\d{6}\b", summary.read_text()))
    return ids
...
def country_table(country: str, cdata: dict) -> str:
    rows = [f"### {country.title()} — {human_status(cdata.get('status'))}", "",
            "| City | PRIDE hits | PXDs listed | In corpus | Screened | Annotated | Blocked |",
            "|---|---:|---:|---:|---:|---:|---:|"]
    tot = dict.fromkeys(("pxd_count", "in_corpus", "screened", "annotated", "blocked"), 0)
    for slug, city in (cdata.get("cities") or {}).items():
        scoped = bool(city.get("pxd_list"))
        listed = city.get("pxd_count", 0) if scoped else "—"
        in_corpus = city.get("in_corpus", 0) if scoped else "—"
        rows.append(f"| {city.get('name', slug)} | {city.get('pride_hits', 0)} | {listed} | {in_corpus} | "
                    f"{city.get('screened', 0)} | {city.get('annotated', 0)} | {city.get('blocked', 0)} |")
        for k in tot:
            if scoped or k not in ("pxd_count", "in_corpus"):
                tot[k] += int(city.get(k, 0) or 0)
    rows.append(f"| **Total** | | **{tot['pxd_count']}** | {tot['in_corpus']} | {tot['screened']} | "
                f"{tot['annotated']} | {tot['blocked']} |")
    return "\n".join(rows)


def render_status_block(cfg: dict) -> str:
    queried = cfg.get("pride_hits_queried", "unknown date")
    intro = (f"One table per country, same columns everywhere. *PRIDE hits* is the rough\n"
             f"full-text hit count from the PRIDE Archive search (unioned over each city's\n"
             f"`search_terms` in `config.yml`, queried {queried}) and only indicates size;\n"
             f"*PXDs listed* is the curated manifest length and is `—` until a\n"
             f"`<country>/<city>.txt` exists; *In corpus* is how many of those already have an\n"
             f"SDRF in [`bigbio/sdrf-annotated-datasets`](https://github.com/{CORPUS_REPO})\n"
             f"(checked against {cfg.get('corpus_source', 'the local checkout')}) and can be skipped.\n"
             f"Regenerate with `python scripts/update_status.py`.")
    tables = [country_table(c, d) for c, d in cfg["countries"].items()]
    return "\n\n".join([intro, *tables])
...
def main() -> None:
    ...
    print(f"\nupdated {CONFIG.name}, README.md status block, status/<country>.md"
